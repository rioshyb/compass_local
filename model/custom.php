<?php
class custom extends spModel
{
	var $pk = "id_cust"; // 每个留言唯一的标志，可以称为主键
	var $table = "custom"; // 数据表的名称
	var $verifier = array( // 验证登录信息，由于密码是加密的输入框生成的，所以不需要进行“格式验证”
		"rules" => array( // 规则
			'name_cust' => array(  // 这里是对uname的验证规则
	...
	public function userlogin($uname, $upass){ 
		$conditions = array(
			'name_cust' => $uname,
			'pswd_cust' => $upass, // 请注意，本例中使用了加密输入框，所以这里的$upass是经过MD5加密的字符串。
		);
		// dump($conditions);
		...
		if( $result = $this->find($conditions) ){ 
			...
			// 用户的角色存储在用户表的acl字段中
			spClass('spAcl')->set($result['acl_cust']); // 通过spAcl类的set方法，将当前会话角色设置成该用户的角色
			$_SESSION["custominfo"] = $result; // 在SESSION中记录当前用户的信息
			return true;
		}else{
	...
	public function acljump(){ 
		...
		if(isset($_SESSION["custominfo"])==true)
		{
